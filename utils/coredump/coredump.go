...
// coredumpResourceOpener implements resourceOpener to provide access inside the coredump
//
// WARNING: this implementation is not actually compliant with the interpreter.ResourceOpener
// interface's requirement of being implemented thread-safe: we currently simply assume that
// with the way that we're calling the process manager in the coredump tests, we can't run into
// these race conditions. This is not a good idea because it relies on implementation details
// of the process manager and should probably be fixed at one point or another.
type coredumpResourceOpener struct {
	process.Process
}

var _ nativeunwind.StackDeltaProvider = &coredumpResourceOpener{}

func (cro *coredumpResourceOpener) GetAndResetStatistics() nativeunwind.Statistics {
	return nativeunwind.Statistics{}
}

func (cro *coredumpResourceOpener) GetIntervalStructuresForFile(_ host.FileID,
	elfRef *pfelf.Reference, interval *sdtypes.IntervalData) error {
	return elfunwindinfo.ExtractELF(elfRef, interval)
}
...
func ExtractTraces(ctx context.Context, pr process.Process, debug bool,
	...
	for _, mapName := range []string{"interpreter_offsets",
		...
		"perl_procs", "py_procs", "hotspot_procs", "ruby_procs", "php_procs",
		"v8_procs"} {
	...
	coredumpOpener := &coredumpResourceOpener{Process: pr}
	...
	includeTracers := make([]bool, config.MaxTracers)
	for i := range includeTracers {
		includeTracers[i] = true
	}
