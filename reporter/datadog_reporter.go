...
// reportProfile creates and sends out an OTLP profile.
func (r *DatadogReporter) reportProfile(ctx context.Context) error {
	...
	err = uploadProfiles([]profileData{{name: "cpu.pprof", data: b.Bytes()}}, time.Unix(int64(startTS), 0), time.Unix(int64(endTS), 0), r.agentAddr, tags)

	return err
}

// getProfile returns an OTLP profile containing all collected samples up to this moment.
func (r *DatadogReporter) getPprofProfile() (profile *pprofile.Profile, startTS uint64, endTS uint64) {
	...
	profile = &pprofile.Profile{
		SampleType:        []*pprofile.ValueType{{Type: "samples", Unit: "count"}, {Type: "cpu", Unit: "nanoseconds"}},
		...
		for i := range trace.frameTypes {
			...
			case libpf.NativeFrame:
				...
				if tmpMapping, exists := fileIDtoMapping[trace.files[i]]; exists {
					...
					tmpMapping := createPprofMapping(profile, uint64(trace.linenos[i]), fileName, trace.files[i].StringNoQuotes())
					fileIDtoMapping[trace.files[i]] = tmpMapping
					loc.Mapping = tmpMapping
				}
				line := pprofile.Line{Function: createPprofFunctionEntry(funcMap, profile, "", trace.comm)}
	...
	log.Infof("Reporting pprof profile with %d samples from %v to %v", totalSampleCount, startTS, endTS)

	profile.DurationNanos = int64(time.Unix(int64(endTS-startTS), 0).UnixNano())
	profile.TimeNanos = int64(time.Unix(int64(startTS), 0).UnixNano())
...
func getDummyMapping(fileIDtoMapping map[libpf.FileID]*pprofile.Mapping,
	profile *pprofile.Profile,
	fileID libpf.FileID) *pprofile.Mapping {
